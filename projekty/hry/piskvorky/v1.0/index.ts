...
const HTMLElm = {	// objekt s prvky, odkazujícími na html element dom objekty
 span:{
   displayingWhoIsTurn: document.getElementById("span-displaying-who-is-turn") as HTMLElement,
 },
 input:{
   dimensionOfPlaygroundX: document.getElementById("input-dimension-of-playground-x") as HTMLInputElement,
   dimensionOfPlaygroundY: document.getElementById("input-dimension-of-playground-y") as HTMLInputElement,
   numberOfWinningCirclesOrCrosses: document.getElementById("input-number-of-winning-circles-or-crosses") as HTMLInputElement,
 },
 button:{
   start: document.getElementById("button-start") as HTMLElement,
 },
 div:{
   ticTacToeGridContainer: document.getElementById("div-tic-tac-toe-grid-container") as HTMLElement,
 },
 style: function(){
          const styleElm=document.createElement("style") as HTMLStyleElement;
...
let HTMLElmCell:HTMLElement[][];			// proměnná, které bude přiřazeno dvourozměrné pole, které bude v prvcích obsahovat odkazy na html element dom objekty každé buňky grid containeru hrací plochy

const value = {		// objekt s hodnotami pro hru
 ...
 circle:{						// hráčem hry je právě "kolečko/circle" a "krížek/cross"; tvary "circle" a "cross" jsou v kódu souhrně nazývány jako "symboly"
 ...
 CSSStyle:{
   strikeLineColor:"rgb(10,10,10)",			// barva, kterou se proškrtává vítězná série koleček/křížků
   circleColor:"blue",					// barva hráče majícího kolečka
   crossColor:"green",
 ...
 whoIsTurn: 0 as 0|1,					// hodnota 0/1 určuje, zda-li táhne kolečko "0" nebo křížek "1"
...
function start (){
  ...
  if ( (HTMLElm.input.dimensionOfPlaygroundX.value as any)*1 < (value.dimensionOfPlaygroundX.min as any)*1 ) HTMLElm.input.dimensionOfPlaygroundX.value=value.dimensionOfPlaygroundX.min;
  if ( (HTMLElm.input.dimensionOfPlaygroundX.value as any)*1 > (value.dimensionOfPlaygroundX.max as any)*1 ) HTMLElm.input.dimensionOfPlaygroundX.value=value.dimensionOfPlaygroundX.max;
  if ( (HTMLElm.input.dimensionOfPlaygroundY.value as any)*1 < (value.dimensionOfPlaygroundY.min as any)*1 ) HTMLElm.input.dimensionOfPlaygroundY.value=value.dimensionOfPlaygroundY.min;
  if ( (HTMLElm.input.dimensionOfPlaygroundY.value as any)*1 > (value.dimensionOfPlaygroundY.max as any)*1 ) HTMLElm.input.dimensionOfPlaygroundY.value=value.dimensionOfPlaygroundY.max;
  if ( (HTMLElm.input.numberOfWinningCirclesOrCrosses.value as any)*1 < (value.numberOfWinningCirclesOrCrosses.min as any)*1 ) HTMLElm.input.numberOfWinningCirclesOrCrosses.value= value.numberOfWinningCirclesOrCrosses.min;
  if ( (HTMLElm.input.numberOfWinningCirclesOrCrosses.value as any)*1 > (value.numberOfWinningCirclesOrCrosses.max as any)*1 ) HTMLElm.input.numberOfWinningCirclesOrCrosses.value= value.numberOfWinningCirclesOrCrosses.max;

  // uložení (uživatelem) aktuálně zvolených hodnot
  value.dimensionOfPlaygroundX.actual= Math.round( (HTMLElm.input.dimensionOfPlaygroundX.value as any)*1 );	// v html je "input type='number'" tj. input hodnotou bude string obsahující číslo, celé i desetinné (s desetinnou tečkou), i obsahující zbytečné nuly na začátku - se zbytečnými nulami si poradí *1 a převede řetězec na číslo bez nul na začátku, necelou část zaokrouhlí "Math.round()"
  value.dimensionOfPlaygroundY.actual= Math.round( (HTMLElm.input.dimensionOfPlaygroundY.value as any)*1 );
  value.numberOfWinningCirclesOrCrosses.actual= Math.round( (HTMLElm.input.numberOfWinningCirclesOrCrosses.value as any)*1 );

  const dimX:number= value.dimensionOfPlaygroundX.actual;
  const dimY:number= value.dimensionOfPlaygroundY.actual;
  ...
  value.whoIsTurn=Math.floor(Math.random()*2) as 0|1;						// náhodná hodnota 0|1 (začíná kolečko nebo křížek)
    if (value.whoIsTurn) displayInHTMLIsTurnOfCrossPlayer();					// před prvním tahem zobrazí, zda-li táhnou kolečka nebo křížky, tj. i kdo začíná
    else displayInHTMLIsTurnOfCirclePlayer();
...
function gridContainerClick (event:Event){
 const target=event.target as HTMLElement
 if (target===HTMLElm.div.ticTacToeGridContainer) return;					// když uživatel klikl na grid container v místě, kde se nenachází buňka/grid_item, tj. klikl do mezery v okolí buněk (tehdy "event.target" odkazuje na grid container, nikoliv buňku/grid_item)
 if ( target.classList.contains(value.CSSClass.drawingCircle) || target.classList.contains(value.CSSClass.drawingCross) ) return;	// když uživatel klikl na grid container v místě, kde se nachází buňka/grid_item, ale v buňce již je nakresleno kolečko nebo křížek

 if (value.whoIsTurn===0) circlePlayerGo(target);						// je-li na tahu hráč mající kolečka
 else crossPlayerGo(target);
}


function circlePlayerGo (clickedCell: HTMLElement) {
 clickedCell.classList.add(value.CSSClass.drawingCircle);					// přidá buňce/grid_itemu css třídu, která v buňce vykreslí kroužek

 const win:boolean= checkWinAndIfStrikeWinningSeriesAndReturnTrue(clickedCell, value.CSSClass.drawingCircle);
...
function crossPlayerGo (clickedCell: HTMLElement) {
 clickedCell.classList.add(value.CSSClass.drawingCross);

 const win:boolean= checkWinAndIfStrikeWinningSeriesAndReturnTrue(clickedCell, value.CSSClass.drawingCross);
 if(win) {
   HTMLElm.span.displayingWhoIsTurn.innerText+=" (vyhrál)";
   HTMLElm.div.ticTacToeGridContainer.removeEventListener("click", gridContainerClick);
   return;
...
function checkWinAndIfStrikeWinningSeriesAndReturnTrue (clickedCell: HTMLElement, CSSClassDrawingSymbolOfPlayerWhoClickedCell: string):boolean {
 const dimX:number= value.dimensionOfPlaygroundX.actual;					// rozměr "x" hrací plochy
 const dimY:number= value.dimensionOfPlaygroundY.actual;
 const clickedCellI:number= (clickedCell.dataset.i as any) *1					// "i" je číslo/index řádku buňky
 const clickedCellJ:number= (clickedCell.dataset.j as any) *1					// "j" je číslo/index sloupce buňky
...
function countWinningSeriesHorizontal (dimX:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 ...
 for(let i=clickedCellI, j=clickedCellJ+1; j<dimX; j++){
   if( HTMLElmCell[i][j].classList.contains(CSSClassDrawingSymbolOfPlayerWhoClickedCell) ) count++;
   else break;
 }
...
function strikeWinningSeriesHorizontal(dimX:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
...
function countWinningSeriesVertical (dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 let count=1;
...
function strikeWinningSeriesVertical(dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
...
function countWinningSeriesLTtoRB (dimX:number, dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 let count=1;
...
function strikeWinningSeriesLTtoRB(dimX:number, dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 for(let i=clickedCellI, j=clickedCellJ; i<dimY && j<dimX; i++, j++){
   if( HTMLElmCell[i][j].classList.contains(CSSClassDrawingSymbolOfPlayerWhoClickedCell) ) {
     HTMLElmCell[i][j].style.position="relative";
     const canvas=document.createElement("canvas"); HTMLElmCell[i][j].appendChild(canvas);
     canvas.setAttribute("width","34px"); canvas.setAttribute("height","34px"); canvas.style.cssText="position:absolute;left:-2px;top:-2px;z-index:1;";
     const ctx=canvas.getContext("2d");
     if(ctx) { ctx.strokeStyle=value.CSSStyle.strikeLineColor; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(34,34); ctx.stroke() }
   }
   else break;
 }
 for(let i=clickedCellI-1, j=clickedCellJ-1; i>-1 && j>-1; i--, j--){
...
function countWinningSeriesRTtoLB (dimX:number, dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 let count=1;
...
function strikeWinningSeriesRTtoLB(dimX:number, dimY:number, clickedCellI:number, clickedCellJ:number, CSSClassDrawingSymbolOfPlayerWhoClickedCell:string){
 for(let i=clickedCellI, j=clickedCellJ; i<dimY && j>-1; i++, j--){
   if( HTMLElmCell[i][j].classList.contains(CSSClassDrawingSymbolOfPlayerWhoClickedCell) ) {
     ...
     canvas.setAttribute("width","34px"); canvas.setAttribute("height","34px"); canvas.style.cssText="position:absolute;left:-2px;top:-2px;z-index:1;";
     const ctx=canvas.getContext("2d");
     if(ctx) { ctx.strokeStyle=value.CSSStyle.strikeLineColor; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(0,34); ctx.lineTo(34,0); ctx.stroke() }
   }
   else break;
 }
 for(let i=clickedCellI-1, j=clickedCellJ+1; i>-1 && j<dimX; i--, j++){
