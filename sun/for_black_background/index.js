...
function setInitialStyleForElement(sunElm) {
    ...
    sunElm.style.height = 0.16 * lesserViewportSize + "px";
    sunElm.style.width = 0.16 * lesserViewportSize + "px";
...
function letThereBeSunOnElement(sunElm) {
    ...
    const blackDistanceAtNight = 27; // yellow is at ${yellowDistance}% (eg. 10%), at night black is at ${blackDistanceAtNight}% (eg. 11%), only ${blackDistanceAtNight - yellowDistance}% (eg. 1%) transition from yellow to black, no much room for sun yellow light
    const blackDistanceAtNoon = 100; // yellow is at ${yellowDistance}%, at noon black is ${blackDistanceAtNoon}% (eg. 40%), ${blackDistanceAtNight - yellowDistance}% (eg. 30%) transition from yellow to black, giving room for sun yellow light
